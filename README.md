...
| 서지인 (부리더) | WebSocket, STOMP, redis를 사용한 실시간 채팅 기능<br> SSE 실시간 알림 기능<br> 담당 기능 테스트 코드 작성<br>담당한 백엔드의 프론트엔드 페이지 작성 및 연결 |
...
<div markdown="1">       
    ...
    - 문의하기 특성상 대부분 채팅은 오래 이어가지 않기 때문에 데이터를 장기적으로 저장하지 않고,
    - 읽기 및 쓰기를 처리할 수 있는 속도가 빠른 Redis가 실시간 채팅 기능에 적합하고 판단

    ### - 기술의 장, 단점

    - Redis
        - 장점
            - 메모리에서 데이터를 처리하기 때문에 작업 속도가 빠르다.
            - 다양한 데이터 구조를 지원하여 다양한 용도로 활용 가능하다.
        - 단점
